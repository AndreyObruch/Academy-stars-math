...
// ============================================================================
// БЛОК 7B: ТРЕНАЖЁР ОШИБОК (MistakeTrainer)
// ============================================================================
...
// ============================================================================
// БЛОК 9: СЮЖЕТНЫЙ ГЕНЕРАТОР (StoryEngine)
// ============================================================================
const StoryEngine = {
BANK: {
addition: [
...
"Экипаж {a} человек пополнился {b} стажёрами. Итого?"
],
subtraction: [
...
comparison: [
"Плотность поля A: {a}, поля B: {b}. Какое больше?",
"Корабль {a} летит быстрее корабля {b}. Какой знак?",
"Энергия щита {a}, энергия меча {b}. Сравни!",
"Запас кислорода {a} и {b}. Где больше?"
],
composition: [
"Экипаж из {n} человек: {a} пилотов и ... штурманов?",
"Груз {n} тонн: {a} тонн руды и ... кристаллов?",
"Корабль {n} метров: {a} метров корпус и ... нос?"
],
visual_count: [
...
place_value: [
"В номере корабля {n}: {tens} десятков и ... единиц?",
"Код шлюза {n}: {tens} десятков и сколько единиц?"
],
addition_20: [
...
"Запас {a} больше запаса {b}. На сколько?"
],
unknown: [
"... + {b} = {c}. Сколько тонн топлива в баке?",
"Корабль взял ещё {b} и стало {c}. Сколько было сначала?",
"Было ..., отдали {b}, осталось {c}. Сколько было?"
],
parity: [
"Число {n} — чётное или нечётное?",
"Двигатель {n}-й модели. Чётный или нечётный номер?"
],
group_count: [
"Сигналы идут через каждые {step} секунд: 0, {s1}, {s2}, ?, {s4}. Что пропущено?",
"Маяки включаются по {step}: 0, {s1}, ?, {s3}, {s4}. Найди пропуск!"
],
measurement: [
"Корпус корабля: {q}. Переведи!",
"Длина скафандра: {q}",
"Грузовой отсек: {q}"
],
pattern: [
"Продолжи последовательность сигналов: {seq}, ...",
"Код доступа: {seq}, ... Какое следующее число?"
],
emoji_rebus: [
...
return settings.storiesEnabled !== false;
},
getStory(type, data) {
const templates = this.BANK[type];
...
.replace(/{s1}/g, data.s1 ?? '')
.replace(/{s2}/g, data.s2 ?? '')
.replace(/{s3}/g, data.s3 ?? '')
.replace(/{s4}/g, data.s4 ?? '')
.replace(/{step}/g, data.step ?? '')
.replace(/{sum}/g, data.sum ?? '');
},
wrapQuestion(question, levelId) {
if (question.story) return question;
const data = this.extractData(question);
const story = this.getStory(question.type, data);
if (story) {
...
const data = {};
const nums = question.question.match(/\d+/g) || [];
switch (question.type) {
case 'visual_count':
case 'visual_sequence':
data.emoji = question.visual ? question.visual[0] : '⭐';
break;
case 'comparison':
data.a = nums[0];
data.b = nums[1];
...
case 'place_value':
data.n = nums[0];
data.tens = Math.floor(nums[0] / 10);
break;
case 'difference':
data.a = nums[0];
data.b = nums[1];
...
break;
case 'group_count': {
const seqMatch = question.question.match(/\d+/g);
if (seqMatch) {
data.seq = seqMatch.join(', ');
data.s1 = seqMatch[1] || '';
data.s2 = seqMatch[2] || '';
data.s3 = seqMatch[3] || '';
data.s4 = seqMatch[4] || '';
data.step = seqMatch[1] ? parseInt(seqMatch[1]) - parseInt(seqMatch[0]) : 2;
}
break;
}
case 'measurement':
...
break;
case 'emoji_rebus': {
const emMatch = question.question.match(/^[^\s+]+/);
data.emoji = emMatch ? emMatch[0] : '';
const sumMatch = question.question.match(/=\s*(\d+)/);
data.sum = sumMatch ? sumMatch[1] : '';
break;
}
case 'pattern': {
const seqP = question.question.match(/\d+(?:,\s*\d+)*/);
...
// ============================================================================
// БЛОК 10: БАНК ОПРЕДЕЛЕНИЙ И АВТОСМЕНА (DefinitionsManager)
// ============================================================================
...
{ title: "Геометрическая фигура", text: "Геометрическая фигура — это форма предмета. Треугольник имеет 3 угла, квадрат — 4 равные стороны." },
...
// ============================================================================
// БЛОК 11: ИГРОВОЙ ПРОЦЕСС (GameManager)
// ============================================================================
const GameManager = {
...
const topic = question.type || 'unknown';
if (!player.stats[topic]) player.stats[topic] = { correct: 0, wrong: 0 };
if (isCorrect) player.stats[topic].correct++;
else player.stats[topic].wrong++;
StateManager.updatePlayer(UIManager.currentPlayer, { stats: player.stats });
},
...
if (isCorrect) {
this.correctCount++;
this.recordStat(this.currentQuestion, true);
...
this.mistakesCount++;
this.recordStat(this.currentQuestion, false);
...
// ============================================================================
// БЛОК 12: УПРАВЛЕНИЕ ИНТЕРФЕЙСОМ (UIManager)
// ============================================================================
const UIManager = {
currentPlayer: null,
selectedGrade: 1,
...
addition: 'Сложение до 10',
subtraction: 'Вычитание до 10',
visual_sequence: 'Последовательности',
place_value: 'Разряды чисел',
addition_20: 'Сложение до 20',
subtraction_20: 'Вычитание до 20',
mixed_20: 'Смешанные до 20',
difference: 'Разностное сравнение',
increase_decrease: 'Увеличить / уменьшить',
unknown: 'Неизвестное число',
parity: 'Чёт и нечет',
group_count: 'Счёт группами',
text_problem_1: 'Задачи простые',
text_problem_2: 'Задачи составные',
measurement: 'Измерения (см, дм, кг, л)',
geometry: 'Геометрия',
andromeda_mix: 'Смешанный блок 1',
odd_one_out: 'Что лишнее',
pattern: 'Закономерности',
emoji_rebus: 'Эмодзи-ребусы',
true_false: 'Верно / неверно',
logic_mix_1: 'Смешанная логика',
trick_question: 'Задачи с подвохом',
logic_chain: 'Логические цепочки',
spatial: 'Пространственное мышление',
riddle: 'Загадки',
detective_mix: 'Детективные задачи',
combinatorics: 'Комбинаторика',
number_puzzle: 'Числовые головоломки',
attention: 'Внимание',
space_riddle: 'Космические загадки',
circus_mix: 'Цирковой микс',
logic_smekalka: 'Логика + смекалка',
number_pattern: 'Числа и закономерности',
space_attention: 'Пространство + внимание',
final_warmup: 'Финальная разминка',
mega_boss: 'МЕГА-экзамен'
},
init() {
ParticleSystem.init();
...
const text4 = document.getElementById('login-text-4');
const text5 = document.getElementById('login-text-5');
const content = document.getElementById('login-content');
[text1, text2, text3, text4, text5].forEach(t => {
...
if (topics.length === 0) {
list.innerHTML = '<p style="color: #a8d8ff; margin-top: 10px;">Данных пока нет — реши несколько задач!</p>';
return;
}
topics.sort((a, b) => {
const totalA = stats[a].correct + stats[a].wrong;
const totalB = stats[b].correct + stats[b].wrong;
...
row.className = 'progress-topic';
row.innerHTML = `<div class="progress-topic-head"><span>${this.TOPIC_NAMES[topic] || topic}</span><span>${percent}% (${s.correct}/${total})</span></div><div class="progress-bar"><div class="progress-fill" style="width: ${percent}%"></div></div>`;
...
if (motivationData.newBadge) {
setTimeout(() => this.showBadgeAward(motivationData.newBadge), 2000);
}
const finish = () => {
this.renderMenuLeaderboard();
...
? parseInt(text) === correct
: text === String(correct);
const isSelected = (typeof selected === 'number')
? parseInt(text) === selected
