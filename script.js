/**
АКАДЕМИЯ ЗВЁЗДНЫХ МАТЕМАТИКОВ v4.0
...
// Микро-задержка для атмосферности
...
UIManager.showFeedbackIcon(true); // <-- ДОБАВЛЕНО: мгновенная визуальная обратная связь
...
UIManager.showFeedbackIcon(false); // <-- ДОБАВЛЕНО: мгновенная визуальная обратная связь
...
// Проверка: есть ли профиль на сервере, если нет — создаём
fetch(`${API_BASE}/api/profile/${playerName}`)
.then(res => {
if (res.status === 404) {
