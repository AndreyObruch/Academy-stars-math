/**
 * АКАДЕМИЯ ЗВЁЗДНЫХ МАТЕМАТИКОВ — v5.0 (Задача 1.1)
 * Сюжетный генератор: оборачивает «сухие» примеры в контекст космической миссии.
 * Подключается в routes/generate.js. Математический движок не изменяется.
 */

// Банк сюжетов по типам задач. Плейсхолдеры подставляются из чисел задачи.
const BANK = {
  visual_count: [
  ...
  unknown: [
    'В баке было топливо. Долили ещё {b} тонн — стало {c}. Сколько было сначала?',
    'Корабль взял груз и стало {c} тонн. До погрузки было ... и добавили {b}. Сколько было?'
  ],
  parity: [
  ...
  measurement: [
    'Инженеры измеряют корпус корабля. Переведи: {q}',
...
// Обернуть задачу в сюжет. Если сюжет уже есть или типа нет в банке — не трогаем.
